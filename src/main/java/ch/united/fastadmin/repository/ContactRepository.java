...
public interface ContactRepository extends JpaRepository<Contact, Long>, JpaSpecificationExecutor<Contact> {
    @Query(
        value = "select distinct contact from Contact contact left join fetch contact.relations",
    ...
    @Query("select distinct contact from Contact contact left join fetch contact.relations")
    List<Contact> findAllWithEagerRelationships();

    @Query("select contact from Contact contact left join fetch contact.relations where contact.id =:id")
