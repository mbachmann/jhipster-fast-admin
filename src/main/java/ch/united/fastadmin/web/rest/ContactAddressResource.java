...
import ch.united.fastadmin.service.ContactAddressQueryService;
...
public class ContactAddressResource {
    ...
    private final ContactAddressQueryService contactAddressQueryService;

    public ContactAddressResource(
        ContactAddressService contactAddressService,
        ContactAddressRepository contactAddressRepository,
        ContactAddressQueryService contactAddressQueryService
    ) {
        this.contactAddressService = contactAddressService;
        this.contactAddressRepository = contactAddressRepository;
        this.contactAddressQueryService = contactAddressQueryService;
    }
    ...
    public ResponseEntity<List<ContactAddressDTO>> getAllContactAddresses(ContactAddressCriteria criteria, Pageable pageable) {
        log.debug("REST request to get ContactAddresses by criteria: {}", criteria);
        Page<ContactAddressDTO> page = contactAddressQueryService.findByCriteria(criteria, pageable);
    ...
    public ResponseEntity<Long> countContactAddresses(ContactAddressCriteria criteria) {
        log.debug("REST request to count ContactAddresses by criteria: {}", criteria);
        return ResponseEntity.ok().body(contactAddressQueryService.countByCriteria(criteria));
    }
    ...
    /**
     * {@code SEARCH  /_search/contact-addresses?query=:query} : search for the contactAddress corresponding
     * to the query.
     *
     * @param query the query of the contactAddress search.
     * @param pageable the pagination information.
     * @return the result of the search.
     */
    @GetMapping("/_search/contact-addresses")
    public ResponseEntity<List<ContactAddressDTO>> searchContactAddresses(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of ContactAddresses for query {}", query);
        Page<ContactAddressDTO> page = contactAddressService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
}
