...
import ch.united.fastadmin.service.ContactGroupQueryService;
...
public class ContactGroupResource {
    ...
    public ContactGroupResource(
        ContactGroupService contactGroupService,
        ContactGroupRepository contactGroupRepository,
        ContactGroupQueryService contactGroupQueryService
    ) {
        this.contactGroupService = contactGroupService;
        this.contactGroupRepository = contactGroupRepository;
        this.contactGroupQueryService = contactGroupQueryService;
    }
    ...
    public ResponseEntity<List<ContactGroupDTO>> getAllContactGroups(ContactGroupCriteria criteria) {
        log.debug("REST request to get ContactGroups by criteria: {}", criteria);
        List<ContactGroupDTO> entityList = contactGroupQueryService.findByCriteria(criteria);
        return ResponseEntity.ok().body(entityList);
    }

    /**
     * {@code GET  /contact-groups/count} : count all the contactGroups.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/contact-groups/count")
    public ResponseEntity<Long> countContactGroups(ContactGroupCriteria criteria) {
        log.debug("REST request to count ContactGroups by criteria: {}", criteria);
        return ResponseEntity.ok().body(contactGroupQueryService.countByCriteria(criteria));
    }
    ...
    /**
     * {@code SEARCH  /_search/contact-groups?query=:query} : search for the contactGroup corresponding
     * to the query.
     *
     * @param query the query of the contactGroup search.
     * @return the result of the search.
     */
    @GetMapping("/_search/contact-groups")
    public List<ContactGroupDTO> searchContactGroups(@RequestParam String query) {
        log.debug("REST request to search ContactGroups for query {}", query);
        return contactGroupService.search(query);
    }
}
