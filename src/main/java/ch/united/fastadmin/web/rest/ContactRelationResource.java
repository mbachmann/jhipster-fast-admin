...
import ch.united.fastadmin.service.ContactRelationQueryService;
...
public class ContactRelationResource {
    ...
    public ContactRelationResource(
        ContactRelationService contactRelationService,
    ...
    public ResponseEntity<List<ContactRelationDTO>> getAllContactRelations(ContactRelationCriteria criteria) {
        log.debug("REST request to get ContactRelations by criteria: {}", criteria);
        List<ContactRelationDTO> entityList = contactRelationQueryService.findByCriteria(criteria);
        return ResponseEntity.ok().body(entityList);
    }

    /**
     * {@code GET  /contact-relations/count} : count all the contactRelations.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/contact-relations/count")
    public ResponseEntity<Long> countContactRelations(ContactRelationCriteria criteria) {
        log.debug("REST request to count ContactRelations by criteria: {}", criteria);
        return ResponseEntity.ok().body(contactRelationQueryService.countByCriteria(criteria));
    }
