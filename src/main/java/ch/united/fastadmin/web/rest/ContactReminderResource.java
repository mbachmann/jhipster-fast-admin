...
public class ContactReminderResource {
    ...
    public ResponseEntity<List<ContactReminderDTO>> getAllContactReminders(ContactReminderCriteria criteria) {
        log.debug("REST request to get ContactReminders by criteria: {}", criteria);
        List<ContactReminderDTO> entityList = contactReminderQueryService.findByCriteria(criteria);
    ...
    /**
     * {@code SEARCH  /_search/contact-reminders?query=:query} : search for the contactReminder corresponding
     * to the query.
     *
     * @param query the query of the contactReminder search.
     * @return the result of the search.
     */
    @GetMapping("/_search/contact-reminders")
    public List<ContactReminderDTO> searchContactReminders(@RequestParam String query) {
        log.debug("REST request to search ContactReminders for query {}", query);
        return contactReminderService.search(query);
    }
}
