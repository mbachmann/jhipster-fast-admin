...
describe('Component Tests', () => {
  describe('ContactMySuffix Management Update Component', () => {
    ...
    describe('ngOnInit', () => {
      ...
      it('Should call ContactRelationMySuffix query and add missing value', () => {
        const contact: IContactMySuffix = { id: 456 };
        const relations: IContactRelationMySuffix[] = [{ id: 7767 }];
        contact.relations = relations;

        const contactRelationCollection: IContactRelationMySuffix[] = [{ id: 47446 }];
        jest.spyOn(contactRelationService, 'query').mockReturnValue(of(new HttpResponse({ body: contactRelationCollection })));
        const additionalContactRelationMySuffixes = [...relations];
        const expectedCollection: IContactRelationMySuffix[] = [...additionalContactRelationMySuffixes, ...contactRelationCollection];
        jest.spyOn(contactRelationService, 'addContactRelationMySuffixToCollectionIfMissing').mockReturnValue(expectedCollection);

        activatedRoute.data = of({ contact });
        comp.ngOnInit();

        expect(contactRelationService.query).toHaveBeenCalled();
        expect(contactRelationService.addContactRelationMySuffixToCollectionIfMissing).toHaveBeenCalledWith(
          contactRelationCollection,
          ...additionalContactRelationMySuffixes
        );
      ...
      it('Should update editForm', () => {
        ...
        contact.mainAddress = mainAddress;
        const relations: IContactRelationMySuffix = { id: 28140 };
        contact.relations = [relations];
    ...
    describe('Tracking relationships identifiers', () => {
      ...
      describe('trackContactRelationMySuffixById', () => {
        it('Should return tracked ContactRelationMySuffix primary key', () => {
          const entity = { id: 123 };
          const trackResult = comp.trackContactRelationMySuffixById(0, entity);
          expect(trackResult).toEqual(entity.id);
        });
      });
    });
    ...
    describe('Getting selected relationships', () => {
      describe('getSelectedContactRelationMySuffix', () => {
        it('Should return option if no ContactRelationMySuffix is selected', () => {
          const option = { id: 123 };
          const result = comp.getSelectedContactRelationMySuffix(option);
          expect(result === option).toEqual(true);
        });

        it('Should return selected ContactRelationMySuffix for according option', () => {
          const option = { id: 123 };
          const selected = { id: 123 };
          const selected2 = { id: 456 };
          const result = comp.getSelectedContactRelationMySuffix(option, [selected2, selected]);
          expect(result === selected).toEqual(true);
          expect(result === selected2).toEqual(false);
          expect(result === option).toEqual(false);
        });

        it('Should return option if this ContactRelationMySuffix is not selected', () => {
          const option = { id: 123 };
          const selected = { id: 456 };
          const result = comp.getSelectedContactRelationMySuffix(option, [selected]);
          expect(result === option).toEqual(true);
