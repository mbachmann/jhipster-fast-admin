...
export class PermissionService {
  ...
  update(permission: IPermission): Observable<EntityResponseType> {
    return this.http.put<IPermission>(`${this.resourceUrl}/${getPermissionIdentifier(permission) as number}`, permission, {
      observe: 'response',
    });
  }

  partialUpdate(permission: IPermission): Observable<EntityResponseType> {
    return this.http.patch<IPermission>(`${this.resourceUrl}/${getPermissionIdentifier(permission) as number}`, permission, {
      observe: 'response',
    });
  }
