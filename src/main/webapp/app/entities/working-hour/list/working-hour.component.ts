...
export class WorkingHourComponent implements OnInit {
  ...
  trackId(index: number, item: IWorkingHour): number {
    return item.id!;
  }

  delete(workingHour: IWorkingHour): void {
    const modalRef = this.modalService.open(WorkingHourDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.workingHour = workingHour;
  ...
  protected onSuccess(data: IWorkingHour[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
