...
class CatalogProductResourceIT {
    ...
    void getAllCatalogProductsByVatIsEqualToSomething() throws Exception {
        // Initialize the database
        catalogProductRepository.saveAndFlush(catalogProduct);
        Vat vat = VatResourceIT.createEntity(em);
        em.persist(vat);
        em.flush();
        catalogProduct.setVat(vat);
        catalogProductRepository.saveAndFlush(catalogProduct);
        Long vatId = vat.getId();

        // Get all the catalogProductList where vat equals to vatId
        defaultCatalogProductShouldBeFound("vatId.equals=" + vatId);

        // Get all the catalogProductList where vat equals to (vatId + 1)
        defaultCatalogProductShouldNotBeFound("vatId.equals=" + (vatId + 1));
    }
