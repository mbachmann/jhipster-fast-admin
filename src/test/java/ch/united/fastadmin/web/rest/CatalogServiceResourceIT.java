...
class CatalogServiceResourceIT {
    ...
    void getAllCatalogServicesByVatIsEqualToSomething() throws Exception {
        ...
        em.persist(vat);
        em.flush();
        catalogService.setVat(vat);
        catalogServiceRepository.saveAndFlush(catalogService);
        Long vatId = vat.getId();

        // Get all the catalogServiceList where vat equals to vatId
        defaultCatalogServiceShouldBeFound("vatId.equals=" + vatId);

        // Get all the catalogServiceList where vat equals to (vatId + 1)
        defaultCatalogServiceShouldNotBeFound("vatId.equals=" + (vatId + 1));
    }
