...
class ContactRelationResourceIT {
    ...
    @Test
    @Transactional
    void getContactRelationsByIdFiltering() throws Exception {
        // Initialize the database
        contactRelationRepository.saveAndFlush(contactRelation);

        Long id = contactRelation.getId();

        defaultContactRelationShouldBeFound("id.equals=" + id);
        defaultContactRelationShouldNotBeFound("id.notEquals=" + id);

        defaultContactRelationShouldBeFound("id.greaterThanOrEqual=" + id);
        defaultContactRelationShouldNotBeFound("id.greaterThan=" + id);

        defaultContactRelationShouldBeFound("id.lessThanOrEqual=" + id);
        defaultContactRelationShouldNotBeFound("id.lessThan=" + id);
    }

    @Test
    @Transactional
    void getAllContactRelationsByContactRelationTypeIsEqualToSomething() throws Exception {
        // Initialize the database
        contactRelationRepository.saveAndFlush(contactRelation);

        // Get all the contactRelationList where contactRelationType equals to DEFAULT_CONTACT_RELATION_TYPE
        defaultContactRelationShouldBeFound("contactRelationType.equals=" + DEFAULT_CONTACT_RELATION_TYPE);

        // Get all the contactRelationList where contactRelationType equals to UPDATED_CONTACT_RELATION_TYPE
        defaultContactRelationShouldNotBeFound("contactRelationType.equals=" + UPDATED_CONTACT_RELATION_TYPE);
    }

    @Test
    @Transactional
    void getAllContactRelationsByContactRelationTypeIsNotEqualToSomething() throws Exception {
        // Initialize the database
        contactRelationRepository.saveAndFlush(contactRelation);

        // Get all the contactRelationList where contactRelationType not equals to DEFAULT_CONTACT_RELATION_TYPE
        defaultContactRelationShouldNotBeFound("contactRelationType.notEquals=" + DEFAULT_CONTACT_RELATION_TYPE);

        // Get all the contactRelationList where contactRelationType not equals to UPDATED_CONTACT_RELATION_TYPE
        defaultContactRelationShouldBeFound("contactRelationType.notEquals=" + UPDATED_CONTACT_RELATION_TYPE);
    }

    @Test
    @Transactional
    void getAllContactRelationsByContactRelationTypeIsInShouldWork() throws Exception {
        // Initialize the database
        contactRelationRepository.saveAndFlush(contactRelation);

        // Get all the contactRelationList where contactRelationType in DEFAULT_CONTACT_RELATION_TYPE or UPDATED_CONTACT_RELATION_TYPE
        defaultContactRelationShouldBeFound(
            "contactRelationType.in=" + DEFAULT_CONTACT_RELATION_TYPE + "," + UPDATED_CONTACT_RELATION_TYPE
        );

        // Get all the contactRelationList where contactRelationType equals to UPDATED_CONTACT_RELATION_TYPE
        defaultContactRelationShouldNotBeFound("contactRelationType.in=" + UPDATED_CONTACT_RELATION_TYPE);
    }

    @Test
    @Transactional
    void getAllContactRelationsByContactRelationTypeIsNullOrNotNull() throws Exception {
        // Initialize the database
        contactRelationRepository.saveAndFlush(contactRelation);

        // Get all the contactRelationList where contactRelationType is not null
        defaultContactRelationShouldBeFound("contactRelationType.specified=true");

        // Get all the contactRelationList where contactRelationType is null
        defaultContactRelationShouldNotBeFound("contactRelationType.specified=false");
    }

    @Test
    @Transactional
    void getAllContactRelationsByContactsIsEqualToSomething() throws Exception {
        // Initialize the database
        contactRelationRepository.saveAndFlush(contactRelation);
        Contact contacts = ContactResourceIT.createEntity(em);
        em.persist(contacts);
        em.flush();
        contactRelation.addContacts(contacts);
        contactRelationRepository.saveAndFlush(contactRelation);
        Long contactsId = contacts.getId();

        // Get all the contactRelationList where contacts equals to contactsId
        defaultContactRelationShouldBeFound("contactsId.equals=" + contactsId);

        // Get all the contactRelationList where contacts equals to (contactsId + 1)
        defaultContactRelationShouldNotBeFound("contactsId.equals=" + (contactsId + 1));
    }

    /**
     * Executes the search, and checks that the default entity is returned.
     */
    private void defaultContactRelationShouldBeFound(String filter) throws Exception {
        restContactRelationMockMvc
     ...
     */
    private void defaultContactRelationShouldNotBeFound(String filter) throws Exception {
        restContactRelationMockMvc
            .perform(get(ENTITY_API_URL + "?sort=id,desc&" + filter))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
