...
class ContactReminderResourceIT {
    ...
    private static final String ENTITY_SEARCH_API_URL = "/api/_search/contact-reminders";
     ...
     *
     * @see ch.united.fastadmin.repository.search.ContactReminderSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    void createContactReminder() throws Exception {
        ...
        assertThat(testContactReminder.getContactId()).isEqualTo(DEFAULT_CONTACT_ID);
        ...
        // Validate the ContactReminder in Elasticsearch
        verify(mockContactReminderSearchRepository, times(1)).save(testContactReminder);
    }
    ...
    void createContactReminderWithExistingId() throws Exception {
        ...
        // Validate the ContactReminder in Elasticsearch
        verify(mockContactReminderSearchRepository, times(0)).save(contactReminder);
    }
    ...
    @Test
    @Transactional
    void getAllContactRemindersByContactIdIsEqualToSomething() throws Exception {
        // Initialize the database
        contactReminderRepository.saveAndFlush(contactReminder);

        // Get all the contactReminderList where contactId equals to DEFAULT_CONTACT_ID
        defaultContactReminderShouldBeFound("contactId.equals=" + DEFAULT_CONTACT_ID);

        // Get all the contactReminderList where contactId equals to UPDATED_CONTACT_ID
        defaultContactReminderShouldNotBeFound("contactId.equals=" + UPDATED_CONTACT_ID);
    }

    @Test
    @Transactional
    void getAllContactRemindersByContactIdIsNotEqualToSomething() throws Exception {
        // Initialize the database
        contactReminderRepository.saveAndFlush(contactReminder);

        // Get all the contactReminderList where contactId not equals to DEFAULT_CONTACT_ID
        defaultContactReminderShouldNotBeFound("contactId.notEquals=" + DEFAULT_CONTACT_ID);

        // Get all the contactReminderList where contactId not equals to UPDATED_CONTACT_ID
        defaultContactReminderShouldBeFound("contactId.notEquals=" + UPDATED_CONTACT_ID);
    }

    @Test
    @Transactional
    void getAllContactRemindersByContactIdIsInShouldWork() throws Exception {
        // Initialize the database
        contactReminderRepository.saveAndFlush(contactReminder);

        // Get all the contactReminderList where contactId in DEFAULT_CONTACT_ID or UPDATED_CONTACT_ID
        defaultContactReminderShouldBeFound("contactId.in=" + DEFAULT_CONTACT_ID + "," + UPDATED_CONTACT_ID);

        // Get all the contactReminderList where contactId equals to UPDATED_CONTACT_ID
        defaultContactReminderShouldNotBeFound("contactId.in=" + UPDATED_CONTACT_ID);
    }

    @Test
    @Transactional
    void getAllContactRemindersByContactIdIsNullOrNotNull() throws Exception {
        // Initialize the database
        contactReminderRepository.saveAndFlush(contactReminder);

        // Get all the contactReminderList where contactId is not null
        defaultContactReminderShouldBeFound("contactId.specified=true");

        // Get all the contactReminderList where contactId is null
        defaultContactReminderShouldNotBeFound("contactId.specified=false");
    }

    @Test
    @Transactional
    void getAllContactRemindersByContactIdIsGreaterThanOrEqualToSomething() throws Exception {
        // Initialize the database
        contactReminderRepository.saveAndFlush(contactReminder);

        // Get all the contactReminderList where contactId is greater than or equal to DEFAULT_CONTACT_ID
        defaultContactReminderShouldBeFound("contactId.greaterThanOrEqual=" + DEFAULT_CONTACT_ID);

        // Get all the contactReminderList where contactId is greater than or equal to UPDATED_CONTACT_ID
        defaultContactReminderShouldNotBeFound("contactId.greaterThanOrEqual=" + UPDATED_CONTACT_ID);
    }

    @Test
    @Transactional
    void getAllContactRemindersByContactIdIsLessThanOrEqualToSomething() throws Exception {
        // Initialize the database
        contactReminderRepository.saveAndFlush(contactReminder);

        // Get all the contactReminderList where contactId is less than or equal to DEFAULT_CONTACT_ID
        defaultContactReminderShouldBeFound("contactId.lessThanOrEqual=" + DEFAULT_CONTACT_ID);

        // Get all the contactReminderList where contactId is less than or equal to SMALLER_CONTACT_ID
        defaultContactReminderShouldNotBeFound("contactId.lessThanOrEqual=" + SMALLER_CONTACT_ID);
    }

    @Test
    @Transactional
    void getAllContactRemindersByContactIdIsLessThanSomething() throws Exception {
        // Initialize the database
        contactReminderRepository.saveAndFlush(contactReminder);

        // Get all the contactReminderList where contactId is less than DEFAULT_CONTACT_ID
        defaultContactReminderShouldNotBeFound("contactId.lessThan=" + DEFAULT_CONTACT_ID);

        // Get all the contactReminderList where contactId is less than UPDATED_CONTACT_ID
        defaultContactReminderShouldBeFound("contactId.lessThan=" + UPDATED_CONTACT_ID);
    }

    @Test
    @Transactional
    void getAllContactRemindersByContactIdIsGreaterThanSomething() throws Exception {
        // Initialize the database
        contactReminderRepository.saveAndFlush(contactReminder);

        // Get all the contactReminderList where contactId is greater than DEFAULT_CONTACT_ID
        defaultContactReminderShouldNotBeFound("contactId.greaterThan=" + DEFAULT_CONTACT_ID);

        // Get all the contactReminderList where contactId is greater than SMALLER_CONTACT_ID
        defaultContactReminderShouldBeFound("contactId.greaterThan=" + SMALLER_CONTACT_ID);
    }
    ...
    void putNewContactReminder() throws Exception {
        ...
        assertThat(testContactReminder.getContactId()).isEqualTo(UPDATED_CONTACT_ID);
        ...
        // Validate the ContactReminder in Elasticsearch
        verify(mockContactReminderSearchRepository).save(testContactReminder);
    }
    ...
    void partialUpdateContactReminderWithPatch() throws Exception {
        ...
        partialUpdatedContactReminder.dateTime(UPDATED_DATE_TIME).description(UPDATED_DESCRIPTION).intervalValue(UPDATED_INTERVAL_VALUE);
    ...
    void deleteContactReminder() throws Exception {
        ...
        // Validate the ContactReminder in Elasticsearch
        verify(mockContactReminderSearchRepository, times(1)).deleteById(contactReminder.getId());
    }

    @Test
    @Transactional
    void searchContactReminder() throws Exception {
        // Configure the mock search repository
        // Initialize the database
        contactReminderRepository.saveAndFlush(contactReminder);
        when(mockContactReminderSearchRepository.search(queryStringQuery("id:" + contactReminder.getId())))
            .thenReturn(Collections.singletonList(contactReminder));

        // Search the contactReminder
        restContactReminderMockMvc
            .perform(get(ENTITY_SEARCH_API_URL + "?query=id:" + contactReminder.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(contactReminder.getId().intValue())))
            .andExpect(jsonPath("$.[*].remoteId").value(hasItem(DEFAULT_REMOTE_ID)))
