...
class DeliveryNoteResourceIT {
    ...
    @Test
    @Transactional
    void getAllDeliveryNotesBySignatureIsEqualToSomething() throws Exception {
        // Initialize the database
        deliveryNoteRepository.saveAndFlush(deliveryNote);
        Signature signature = SignatureResourceIT.createEntity(em);
        em.persist(signature);
        em.flush();
        deliveryNote.setSignature(signature);
        deliveryNoteRepository.saveAndFlush(deliveryNote);
        Long signatureId = signature.getId();

        // Get all the deliveryNoteList where signature equals to signatureId
        defaultDeliveryNoteShouldBeFound("signatureId.equals=" + signatureId);

        // Get all the deliveryNoteList where signature equals to (signatureId + 1)
        defaultDeliveryNoteShouldNotBeFound("signatureId.equals=" + (signatureId + 1));
    }
