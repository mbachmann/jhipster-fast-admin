...
describe('ContactAccount e2e test', () => {
  const contactAccountPageUrl = '/contact-account-fa';
  const contactAccountPageUrlPattern = new RegExp('/contact-account-fa(\\?.*)?$');
  ...
  it('should load ContactAccounts', () => {
    cy.visit('/');
    cy.clickOnEntityMenuItem('contact-account-fa');
  ...
  it('should create an instance of ContactAccount', () => {
    ...
    cy.get(`[data-cy="type"]`).select('IBAN');
