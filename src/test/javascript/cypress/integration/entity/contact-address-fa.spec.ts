...
describe('ContactAddress e2e test', () => {
  const contactAddressPageUrl = '/contact-address-fa';
  const contactAddressPageUrlPattern = new RegExp('/contact-address-fa(\\?.*)?$');
  ...
  it('should load ContactAddresses', () => {
    cy.visit('/');
    cy.clickOnEntityMenuItem('contact-address-fa');
  ...
  it('should create an instance of ContactAddress', () => {
    ...
    cy.get(`[data-cy="country"]`).type('Gabon').should('have.value', 'Gabon');
