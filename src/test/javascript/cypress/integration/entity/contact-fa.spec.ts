...
describe('Contact e2e test', () => {
  ...
  it('should create an instance of Contact', () => {
    ...
    cy.get(`[data-cy="currency"]`).type('Virginia').should('have.value', 'Virginia');

    cy.get(`[data-cy="ebillAccountId"]`).type('Bangladesh monitor').should('have.value', 'Bangladesh monitor');

    cy.get(`[data-cy="vatIdentification"]`).type('Associate').should('have.value', 'Associate');

    cy.get(`[data-cy="vatRate"]`).type('17220').should('have.value', '17220');

    cy.get(`[data-cy="discountRate"]`).type('49754').should('have.value', '49754');

    cy.get(`[data-cy="discountType"]`).select('IN_PERCENT');

    cy.get(`[data-cy="paymentGrace"]`).type('79774').should('have.value', '79774');

    cy.get(`[data-cy="hourlyRate"]`).type('33411').should('have.value', '33411');

    cy.get(`[data-cy="created"]`).type('2021-08-05T01:50').should('have.value', '2021-08-05T01:50');

    cy.get(`[data-cy="mainAddressId"]`).type('57216').should('have.value', '57216');

    cy.setFieldSelectToLastOfEntity('contactMainAddress');
