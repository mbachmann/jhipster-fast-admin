...
describe('ContactGroup e2e test', () => {
  const contactGroupPageUrl = '/contact-group-my-suffix';
  const contactGroupPageUrlPattern = new RegExp('/contact-group-my-suffix(\\?.*)?$');
  ...
  it('should load ContactGroups', () => {
    cy.visit('/');
    cy.clickOnEntityMenuItem('contact-group-my-suffix');
  ...
  it('should create an instance of ContactGroup', () => {
    ...
    cy.get(`[data-cy="name"]`).type('policy facilitate Sports').should('have.value', 'policy facilitate Sports');
