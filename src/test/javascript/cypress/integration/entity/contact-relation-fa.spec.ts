...
describe('ContactRelation e2e test', () => {
  const contactRelationPageUrl = '/contact-relation-fa';
  const contactRelationPageUrlPattern = new RegExp('/contact-relation-fa(\\?.*)?$');
  ...
  it('should load ContactRelations', () => {
    cy.visit('/');
    cy.clickOnEntityMenuItem('contact-relation-fa');
