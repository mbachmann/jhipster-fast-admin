...
describe('CustomField e2e test', () => {
  const customFieldPageUrl = '/custom-field-my-suffix';
  const customFieldPageUrlPattern = new RegExp('/custom-field-my-suffix(\\?.*)?$');
  ...
  it('should load CustomFields', () => {
    cy.visit('/');
    cy.clickOnEntityMenuItem('custom-field-my-suffix');
  ...
  it('should create an instance of CustomField', () => {
    ...
    cy.get(`[data-cy="domainResource"]`).select('CATALOG');
    ...
    cy.get(`[data-cy="value"]`).type('grow cross-platform').should('have.value', 'grow cross-platform');
