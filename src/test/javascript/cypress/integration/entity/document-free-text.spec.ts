...
describe('DocumentFreeText e2e test', () => {
  const documentFreeTextPageUrl = '/document-free-text';
  const documentFreeTextPageUrlPattern = new RegExp('/document-free-text(\\?.*)?$');
  ...
  it('should load DocumentFreeTexts', () => {
    cy.visit('/');
    cy.clickOnEntityMenuItem('document-free-text');
