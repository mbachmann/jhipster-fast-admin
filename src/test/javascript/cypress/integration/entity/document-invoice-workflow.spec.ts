...
describe('DocumentInvoiceWorkflow e2e test', () => {
  const documentInvoiceWorkflowPageUrl = '/document-invoice-workflow';
  const documentInvoiceWorkflowPageUrlPattern = new RegExp('/document-invoice-workflow(\\?.*)?$');
  ...
  it('should load DocumentInvoiceWorkflows', () => {
    cy.visit('/');
    cy.clickOnEntityMenuItem('document-invoice-workflow');
