...
describe('DocumentPosition e2e test', () => {
  const documentPositionPageUrl = '/document-position';
  const documentPositionPageUrlPattern = new RegExp('/document-position(\\?.*)?$');
  ...
  it('should load DocumentPositions', () => {
    cy.visit('/');
    cy.clickOnEntityMenuItem('document-position');
