...
describe('Permission e2e test', () => {
  const permissionPageUrl = '/permission-fa';
  const permissionPageUrlPattern = new RegExp('/permission-fa(\\?.*)?$');
  ...
  it('should load Permissions', () => {
    cy.visit('/');
    cy.clickOnEntityMenuItem('permission-fa');
  ...
  it('should create an instance of Permission', () => {
    ...
    cy.get(`[data-cy="edit"]`).select('ALL');

    cy.get(`[data-cy="manage"]`).select('OWN');

    cy.get(`[data-cy="domainArea"]`).select('ACTIVITES');
