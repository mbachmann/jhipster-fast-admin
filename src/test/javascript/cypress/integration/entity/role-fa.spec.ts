...
describe('Role e2e test', () => {
  const rolePageUrl = '/role-fa';
  const rolePageUrlPattern = new RegExp('/role-fa(\\?.*)?$');
  ...
  it('should load Roles', () => {
    cy.visit('/');
    cy.clickOnEntityMenuItem('role-fa');
  ...
  it('should load details Role page', function () {
  ...
  it('should load edit Role page', function () {
    cy.visit(rolePageUrl);
    cy.wait('@entitiesRequest').then(({ response }) => {
      if (response.body.length === 0) {
        this.skip();
  ...
  it('should create an instance of Role', () => {
    ...
    cy.get(`[data-cy="name"]`).type('Illinois').should('have.value', 'Illinois');
  ...
  it('should delete last instance of Role', function () {
    cy.visit(rolePageUrl);
    cy.wait('@entitiesRequest').then(({ response }) => {
      if (response.body.length > 0) {
